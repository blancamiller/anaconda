...
if gp.learn_ard,
    iwgrad =
    sum(
    bsxfun(@times, sum(gradmat,1)', dalphadiw),1)
    - ...
    0.5*
    permute(
    sum(
    bsxfun(@times,
    sum(
    bsxfun(@times,gradmat,delta.^2 + noise_var),1),alpha'),2),[1 3 2]) 
    + ...
    permute(
    sum(
    bsxfun(@times,
    ...
    bsxfun(@times,
    permute(dbar,[1 3 2]),
    bsxfun(@times,delta,K)),1),alpha'),2),[1 3 2])
    + ...
    0.5*
    permute(
    sum(
    ...
    sum(
    bsxfun(@times,Hfo,
    permute(info.gh,[1 2 4 3])),4),[1 4 3 2])),3),[1 2 4 3])...
                                 )),K),1),alpha'),2),[1 3 2]);
    %{
    for k=1:F,        

...
fprintf('highway_sample_controls called: %i times\n', i)
...
fprintf('\n **** highwayfeatures is (mostly) executing correctly **** \n')
...
%fprintf('mdp_data.lanes\n')
%fprintf('length of mdp_data.lanes: %d \n', length(mdp_data.lanes))
%display(mdp_data.lanes)
...
%fprintf('Theta & Car Length\n')  %%%%%%%%%%%%%%%%%% fprintf before this... 
%display(length(reward.theta))
%display(length(cars))
ist = length(reward.theta)-length(cars);

% ------------------------------------------------------- %
% ------------- IF STATEMENT BELOW NOT USED ------------- %
if strcmp(mdp_params.carfile,'none'),
    ...
    %fprintf('1st CAR \n')
    %display(mdp_data.cars{1})
    %fprintf('2nd CAR \n')
    %display(mdp_data.cars{1})
    
    % Check if file has correct number of cars, if not, throw error message
    if length(loaded_cars) ~= length(mdp_data.cars),
        error('Specified car file has the wrong number of cars!');
    end;
    
    % Add car data into mdp, reward, and features
    mdp_data.cars = loaded_cars;
    %fprintf('MDP Data: %d\n', mdp_data.cars{6}.x0)
    
    fprintf('Looping through loaded cars\n')
    # Set features_pt states 
    ...
    %{
    fprintf('Features Point\n')
    fprintf('ist + 1: %d \n', (ist+1))
    display(reward.features{ist+1}.x)
    fprintf('ist + 2: %d \n', ist+2)
    display(reward.features{ist+2}.x)
    fprintf('ist -2 + 1: %d \n', ist -2 +1)
    display(features_pt{ist-2+1}.x)
    fprintf('ist -2 + 2: %d \n', ist-2+2)
    display(features_pt{ist-2+2}.x)
    %}
...
%{
fprintf('= = = = > Highway CAR features called.')

fprintf(' --- Features Pt 1 --- \n')
display(features_pt{1})
fprintf(' --- Features Pt 2 --- \n')
display(features_pt{2})
fprintf(' --- Features Pt 3 --- \n')
display(features_pt{3})
fprintf(' --- Features Pt 4 --- \n')
display(features_pt{4})
%}
...
if mdp_params.group_lane_features,
    ...
    fprintf('ARE THE PARAMS THE SAME HERE ?')
    fieldnames(mdp_params)
